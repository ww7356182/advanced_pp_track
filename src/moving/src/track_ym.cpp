...
/*tar_theta[0]=1.219285;
 ...
 for(int curv_count=1;curv_count<257;curv_count++)
 {
     double d_theta,d_dis;
     d_theta=tar_theta[curv_count]-tar_theta[curv_count-1];
     d_dis=sqrt((tar_x[curv_count]-tar_x[curv_count-1])*(tar_x[curv_count]-tar_x[curv_count-1])+(tar_y[curv_count]-tar_y[curv_count-1])*(tar_y[curv_count]-tar_y[curv_count-1]));
      if(d_theta>180)
  {
    d_theta=d_theta-360;
  }
  if(d_theta<-180)
  {
    d_theta=d_theta+360;
     ...
     curv[curv_count]=d_dis/d_theta*57.2;
     //printf("%f\n",d_dis);
 }
 curv[0]=curv[1];
 curv[257]=curv[256];
 for(int pri_count=0;pri_count<258;pri_count++)
 {
   //printf("curv[%d]=%f\n",pri_count,curv[pri_count]);
 }
...
//发布AGV车身姿态，目标点
    ...
    marker3.pose.position.x = tar_x[min_ind];
    marker3.pose.position.y = tar_y[min_ind];
 ...
 printf("num:%d,error:%f,angle:%f\n",num_,error,angle_delta);
...
double theta_minus(const double& theta_1,const double& theta_2)
{
  int theta_result;
  theta_result=theta1-theta2;
  if(theta_result>180)
  {
    theta_result=theta_result-360;
  }
  if(theta_result<-180)
  {
    theta_result=theta_result+360;
...
//全局最小距离
void dec_min(const double& x_,const double& y_)
{
