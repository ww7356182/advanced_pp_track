...
using namespace Eigen;
  double tar_x[500];    //目标轨迹
  double tar_y[500];
  double tar_theta[500];
  double curv[500];    //轨迹曲率（）
  int count_path;
  int angle;    
  double cur_angle;
  double target_x;  //目标点
...
void dec_min(const double& x_,const double& y_,double* tar_x,double* tar_y) //（寻找当前目标点（距离最近）方法：pp）
{
...
void find_min_ind(const double& x_init,const double& y_init,const double& yaw)  //最近点的索引号
{
  int ind_temp[300];
  int find_j,find_ind;
  double d_fro,d_rear;
...
while (ros::ok())
  {
    ...
    try
    {
      listener.lookupTransform("/map", "/laser",
                               ros::Time(0), transform);  //在cartographer发布的tf树中抓取车的坐标
    }
...
//目标点初始化
  ...
  re_x[re_count]=x;
  re_y[re_count]=y;
  re_d[re_count]=error_ins;  //可视化
  
  
  //设定线速度 考虑曲率，实车跑不需要考虑曲率
  ...
  msg_twist.angular.z=2*v_pp*v_result[1]/(d*d);    //pp
 

//轨迹终点停车
  if(num_==p_size && d<0.8) //d是停车阈值
  {
...
//计算AGV前轮转速  （防疫机器人）
 v1=fabs(msg_twist.angular.z*(r-0.2864));
 v2=fabs(msg_twist.angular.z*(r+0.2864));
 }
 else
 {
   theta1=0;
...
//设置CAN通信
 ...
 canInitializeLibrary();
  ...
  canSetBusParams(h,BAUD_1M,0,0,0,0,0); //设置波特率
  canBusOn(h);
...
//将转速转角存入buffer中
 ...
 if(msg_twist.angular.z<=0)  //z是角速度 负左正右
 {
    ...
    *((int16_t *)&(buffer[2]))=can_angle;
    ...
    //buffer[0]=can_v/256;
    //buffer[1]=can_v%256;
    buffer[0]=int((v2*1560)/(3.1415*0.217*100));
    buffer[1]=(v1*1560)/(3.1415*0.217*100);
    buffer[2]=1;
    buffer[3]=1;
 }
 else
 {
    ...
    *(int16_t*)&buffer[2]=can_angle;
    ...
    //buffer[0]=can_v/256;
    //buffer[1]=can_v%256;
    buffer[0]=(v1*1560)/(3.1415*0.217*100);
    buffer[1]=(v2*1560)/(3.1415*0.217*100);
    buffer[2]=1;
    buffer[3]=1;
 }
