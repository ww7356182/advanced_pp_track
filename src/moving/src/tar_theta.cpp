...
int count,i,j,count_trim;
int k;
double d_temp,d_trim;
/*for(count=0;count<108;count++)
{
    if(tar_x[count+1]-tar_x[count]>0)
    {
        tar_theta[count]=atan2((tar_y[count+1]-tar_y[count])/(tar_x[count+1]-tar_x[count]))*57.29578;
    }
    if(tar_x[count+1]-tar_x[count]<0 &&tar_y[count+1]-tar_y[count]<0)
    {
        tar_theta[count]=atan2((tar_y[count+1]-tar_y[count])/(tar_x[count+1]-tar_x[count]))*57.29578-180;
    }
    if(tar_x[count+1]-tar_x[count]<0 &&tar_y[count+1]-tar_y[count]>0)
    {
        tar_theta[count]=atan2((tar_y[count+1]-tar_y[count])/(tar_x[count+1]-tar_x[count]))*57.29578+180;
...
for(i=1;i<132;i++)
{
    d_temp=sqrt((tar_x[i]-trim_x[count_trim])*(tar_x[i]-trim_x[count_trim])+(tar_y[i]-trim_y[count_trim])*(tar_y[i]-trim_y[count_trim]));
    if(d_temp>0.1)
    {
       trim_x[count_trim+1]=tar_x[i];
       trim_y[count_trim+1]=tar_y[i];
       count_trim++;
    }
}
//printf("%d",count_trim);*/
...
for(int count_theta=0;count_theta<68;count_theta++)
{
//printf(" tar_theta[%d]=%f;\n",count_theta,tar_theta[count_theta]);
}
/*
for(j=0;j<468;j++)
{
    d_temp=sqrt((tar_x[j+1]-tar_x[j])*(tar_x[j+1]-tar_x[j])+(tar_y[j+1]-tar_y[j])*(tar_y[j+1]-tar_y[j]));
    if(d_temp>0.2)
    {
        trim_x[k]=tar_x[j+1];
        k++;
