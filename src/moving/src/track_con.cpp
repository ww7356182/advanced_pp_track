...
if(theta<10 && theta>0)
 ...
 v3=fabs(msg_twist.angular.z*sqrt(pow(r+0.58,2)+pow(0.71,2)));
 }
 else
 {
   theta1=0;
 ...
 int_theta3=round(theta3*100)+6000;
 int_v1=round(v1*1000);
 int_v3=round(v3*1000);
 buffer[0]=int_theta3/256;
 buffer[1]=int_theta3%256;
 buffer[2]=int_theta1/256;
 buffer[3]=int_theta1%256;
 buffer[4]=int_v3/256;
 buffer[5]=int_v3%256; 
 buffer[6]=int_v1/256;
 buffer[7]=int_v1%256;
...
private:
  ...
  int int_theta1,int_theta3,int_v1,int_v3;
  unsigned int dlc;
