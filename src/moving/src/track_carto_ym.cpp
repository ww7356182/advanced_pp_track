...
//ros节点初始化
  ros::init(argc, argv, "track_carto_ym");
  ros::NodeHandle n_; 
...
//设置恒定的线速度
  msg_twist.linear.x=0.8;
  msg_twist.angular.z=2*msg_twist.angular.z;
//轨迹终点停车
  if(num_==257 && d<0.4)
  {
...
//ROS_INFO("d:%f\n num:%d\n,target_x:%f\n target_y:%f\n",d,num_,target_x,target_y);
 printf("num:%d,err:%f,theta2:%f,v:%f,d_ym:%f\n",num_,error,theta2,msg_twist.linear.x,d_ym);
