...
for(count_path=0;count_path<p_size+1;count_path++)
 ...
 v_pp=0.8;  
  ...
  if(curv[min_ind]>0.2|| curv[min_ind]==0.2)
  {
    d_curv=0.6;
    v_pp=0.6;
  }
  d_ym=d_curv+0.3*d_min;
  v_pp=1.5;
  //d_ym=1;
  target_x=tar_x[num_];
  target_y=tar_y[num_];
...
//发布AGV车身姿态，目标点
 ...
 //printf("num:%d,error:%f,angle:%f,ang_l:%f\n",num_,error,angle_delta,theta1);
 //printf("num:%d,cur:%f,d_ym:%f,v_pp:%f\n",num_,curv[min_ind],d_ym,v_pp);
 //printf("%f\n",error);
...
int dec_min(const double& x_,const double& y_)
{
    ...
    for(int dec_i=1;dec_i<p_size;dec_i++)
    {
...
private:
  ...
  geometry_msgs::Quaternion q;
   ...
   double d_min_init,d_tar;
   int error_count;
   double error,sum_er;
   double sum_angle_delta,angle_delta;
   double rt_angle,fun_a,fun_b;
   int sec_min_ind;
   double error_ins;
   double v_pp;
   int p_size;
...
int main(int argc, char **argv)
{
  //static double d;
  //Initiate ROS
  ros::init(argc, argv, "old_pp");
  //Create an object of class SubscribeAndPublish that will take care of everything
  SubscribeAndPublish SAPObject=SubscribeAndPublish();
  //ROS_INFO("callback\n");
  ros::spin();
  SAPObject.show_x(SAPObject.re_count);
  printf("end\n");
  SAPObject.show_y(SAPObject.re_count);
  printf("end\n");
  SAPObject.show_d(SAPObject.re_count);
  printf("end\n");
  SAPObject.show_w(SAPObject.re_count);
  //canBusOff(h);   
  //canClose(h);
  return 0;
}